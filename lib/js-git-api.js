...
module.exports = {
  ...
  clone: function(remote, targetPath, options, callback) {
    ...
    function addRef(err) {
      ...
      repo.updateRef(ref, hash, function(err) {
        ...
        fs.writeFile(headPath, 'ref: ' + ref, createConfig);
      });
    }
  ...
  readRef: function(repo, ref, callback) {
    ...
    repo.readRef(ref, function(err, hash) {
      if (!err && !hash) {
        var wrongPath = path.join(repo.rootPath, ref);
        return fs.readFile(wrongPath, callback); // throw ENOENT
      } else {
  ...
  loadTree: function(repo, ref, callback) {
    ...
    this.readRef(repo, ref, function (err, hash) {
      if (err) return callback(err);
      self.readHead(repo, function(err, head) {
        if (err) return callback(err);
        if (hash == head) return callback();
